...
export default {
  ...
  // 解决方案：监听地址栏参数的变化，执行下面代码
  watch: {
    // data = {a:{b:10}}  'a.b':function(){}
    // （只要是this能够获取到的数据都可以进行监听）字段名称：function(newVal,oldVal){ //当数据改变就会执行  }
  ...
  methods: {
    ...
    // 修改或存入草稿
    async update (draft) {
      // 地址栏添加id地址传参 ?后面键值对query传参 ,后面body传参
      await this.$http.put(
    ...
    // 切换发布与修改
    toggleArticleStatus () {
      // 判断当前是否是编辑(有ID)还是新建(无ID)（判断地址栏是否存在 ID 即可）
      const articleId = this.$route.query.id
      if (articleId) {
        // 获取当前文章信息
        this.getArticle(articleId)
      } else {
      // 重置数据不能为空对象  模版中articleForm.cover.images会报错
        this.articleForm = {
          title: null,
          content: null,
          cover: {
